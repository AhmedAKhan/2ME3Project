...
public class ConnectFourView extends JFrame {
    ...
    private void setupBoard(ConnectFourModel.Slot[][] b, ConnectFourModel.GameState gs, int width, int height){
        ...
        //set the size of all the buttons
    ...
    private void drawCustomGameScreen(Graphics g){
        ...
        System.out.println("the length is " + "ERROR: Red floating disc detected at row: 5 column: 5".length());
    ...
    private void drawTilesFromBoardConfiguration(Graphics g, ConnectFourModel.Slot[][] slotConfiguration){      
        ...
        int widthOfBoard = getWidthOfBoard();//(config[0].length)*k + spaceBetweenSlots;//this is the width of the board
        int heightOfBoard =  getHeightOfBoard();//(numberOfRows+1)*k + spaceBetweenSlots;//the is the height of the board
